...
export default function App(){
  ...
  async function refreshProds(){
    setLoading(true)
    // Tenta ordenar por prioridade (se coluna existir), senão por created_at
    const r1 = await supa.from('productions').select('*')
      .order('priority',{ascending:true, nullsFirst:false})
      .order('deadline',{ascending:true})
    if(!r1.error){ setProductions(r1.data||[]); setLoading(false); return }
    const r2 = await supa.from('productions').select('*').order('created_at',{ascending:true})
    if(r2.error){ alert('Erro ao carregar produções.'); setProductions([]) } else { setProductions(r2.data||[]) }
    setLoading(false)
  }
  ...
  async function signIn(username, password){
    ...
    const { data, error } = await supa
      ...
      .eq('username', u).eq('password', p).eq('approved', true)
      .maybeSingle()
  ...
  async function addProduction(p){
    // tenta com priority; se der erro (coluna não existe), cadastra sem
  ...
  async function approveUser(user, approve){
    if(approve) await supa.from('app_users').update({ approved:true }).eq('id', user.id)
    else await supa.from('app_users').delete().eq('id', user.id)
  }
  async function deleteUser(user){ await supa.from('app_users').delete().eq('id', user.id) }
  ...
  async function bumpPriority(p, dir){
    try{
      ...
      alert('Para mover na fila, crie a coluna priority no Supabase (use o schema.sql).')
    }
    refreshProds()
  }
  ...
  async function togglePause(p){
    ...
    await supa.from('events').insert({ type: paused ? 'resume' : 'pause', production_id:p.id })
    refreshProds()
  }
  ...
  async function sendToNextArea(p){
    ...
    <div>
      <div className="header">
        <div className="header-inner">
          <div className="small" style={{opacity:.6}}>UI • Clean V1</div>
          <div className="brand">
...
function Login({ onSignIn, onRegister }){
  ...
  const [rName,setRName] = useState(''), [rUser,setRUser] = useState(''), [rPass,setRPass] = useState(''), [rArea,setRArea] = useState('')
    ...
    <div className="login-shell">
      <div className="login-card">
        ...
        <div className="between mb12">
          ...
          <button className="btn secondary" onClick={()=>setOpenReg(!openReg)}>{openReg?'Fechar':'Novo cadastro'}</button>
        </div>
