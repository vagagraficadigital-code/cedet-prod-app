...
export default function App(){
  ...
  async function refreshProds(){
    ...
    try {
      ...
      if (error) { alert('Erro ao carregar produções.'); setProductions([]) } else { setProductions(data || []) }
  ...
  async function addProduction(p){
    ...
    let res = await supa.from('productions').insert(item)
    if(res.error){
      const {priority, ...rest} = item
      res = await supa.from('productions').insert(rest)
      err = res.error
    }
  ...
  async function bumpPriority(p, dir){
    try{
      ...
      if(error) throw error
    }catch{ alert('Para mover na fila, crie a coluna priority no Supabase (use o SQL enviado).') }
    await refreshProds()
  }
...
function Login({ onSignIn, onRegister }){
  ...
  return (
    <div className="login-shell">
      <div className="card p24 login-card">
        <div className="flex mb12">
          <img src="/logo.png" className="logo" alt="logo"/>
          <div>
            <h3 className="login-title">Entrar no CEDET</h3>
            <div className="login-sub">Acesse sua conta ou solicite novo cadastro</div>
          </div>
        </div>
