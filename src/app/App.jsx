...
export default function App(){
  ...
  async function bumpPriority(p, dir){
    ...
    await supa.from('productions').update({ priority:newPr }).eq('id', p.id)
    await refreshProds()
  }
  ...
  async function startProd(p){
    ...
    await supa.from('events').insert({ type:'prod_start', production_id:p.id })
    refreshProds()
  }
  async function togglePause(p){
    const isPaused = p.status==='pausado'
    const newStatus = isPaused ? 'em produção' : 'pausado'
    await supa.from('productions').update({ status:newStatus }).eq('id', p.id)
    await supa.from('events').insert({ type: isPaused ? 'resume':'pause', production_id:p.id })
    refreshProds()
  }
  ...
  async function sendToNextArea(p, nextArea){
    ...
    await supa.from('events').insert({ type:'handoff', production_id:p.id, details:{ to: nextArea } })
    refreshProds()
  }
    ...
    <div className="container">
      ...
      <div className="grid2 mb16">
        ...
        <div className="card p16">
          ...
          <div className="list">
            {productions.map(p=> (
              <div key={p.id} className="between card p16 mb12" style={{cursor:'pointer'}} onClick={(e)=>{ if(e.target.closest('.rowBtns')) return; openDetail(p) }}>
                <div>
                ...
                <div className="row rowBtns">
                  <button className="btn" title="Subir na fila" onClick={()=>moveUp(p)}>↑</button>
                  <button className="btn" title="Descer na fila" onClick={()=>moveDown(p)}>↓</button>
                  <button className="btn danger" onClick={(ev)=>{ ev.stopPropagation(); if(confirm('Deletar esta produção?')) deleteProduction(p.id) }}>Deletar</button>
                </div>
              </div>
            ))}
...
function Operator({ productions, startPrep, startProd, togglePause, finishProd, sendToNextArea, signalProblem, openDetail, reload, loading }){
  ...
  const chipsFor = (p)=>{
    const i = areaIdx(p.current_area)
    const prev = i>0 ? AREAS[i-1] : null
    const curr = p.current_area || '—'
    const next = i>=0 && i<AREAS.length-1 ? AREAS[i+1] : null
        ...
        <div className="list">
          {visible.map(p=> {
            ...
            <div key={p.id} className="card p16 mb12">
              ...
              <div className="roww" style={{marginTop:12, alignItems:'center'}}>
                {prev && <span className="area-chip">{prev}</span>}
                <span className="area-chip" style={{background:'#3b1116', borderColor:'#5a1e26'}}>• {curr} •</span>
  ...
  return (
    <div className="container">
      ...
      <div className="card p16">
        ...
        {productions.map(p=> (
          <div key={p.id} className="between mb12" style={{cursor:'pointer'}} onClick={()=>openDetail(p)}>
            <div>
              <div style={{display:'flex',gap:8,alignItems:'center',flexWrap:'wrap'}}>
                <b>{p.title}</b> <span className="small muted">ISBN {p.isbn}</span> <DueBadge deadline={p.deadline}/>
              </div>
...
function BookDetail({ prod, onClose }){
  ...
  const timeline = events.map(ev=>{
    const label = ({
      prep_start: 'Preparação iniciada',
      prod_start: 'Produção iniciada',
      pause: 'Pausa',
      resume: 'Retomada',
      finish: 'Finalizado',
      problem: 'Problema',
  ...
  return (
    <div style={{position:'fixed', inset:0, background:'rgba(0,0,0,0.5)', display:'grid', placeItems:'center', zIndex:1000}}>
      <div className="card p24" style={{width:'min(900px, 95vw)', maxHeight:'90vh', overflow:'auto'}}>
        ...
        <div className="mb12">
          <div style={{display:'flex',gap:8,alignItems:'center',flexWrap:'wrap'}}>
            <h3 style={{margin:'6px 0'}}>{prod.title}</h3> <DueBadge deadline={prod.deadline}/>
          </div>
          <div className="small muted">
            ISBN {prod.isbn} • OS {prod.os_number} • {prod.qty} un. • {prod.pages} págs • Formato {prod.format}
          </div>
        ...
        <div className="card p16">
          ...
          {timeline.length===0 && <div className="small muted">Sem eventos registrados ainda.</div>}
          {timeline.map((t,i)=>(
            <div key={i} className="mb12">
              <div><b>{t.label}</b> <span className="small muted">({fmtDateTime(t.when)})</span></div>
              {t.details && <pre style={{whiteSpace:'pre-wrap', fontSize:12, opacity:.9}}>{JSON.stringify(t.details)}</pre>}
            </div>
          ))}
        </div>
      </div>
